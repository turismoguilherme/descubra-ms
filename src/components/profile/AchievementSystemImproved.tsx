...
const AchievementSystemImproved: React.FC<AchievementSystemProps> = ({
  achievements,
  pantanalAnimals
}) => {
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {unlockedAchievements.slice(-6).map((achievement) => (
                <div 
                  key={achievement.id}
                  className={`p-4 rounded-lg border-2 ${getRarityColor(achievement.rarity)} shadow-md`}
                >
...
export default AchievementSystemImproved;
