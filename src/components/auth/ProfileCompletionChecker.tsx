...
const ProfileCompletionChecker: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    if (!loading && user) {
      ...
      const currentTenant = pathSegments[0]; // 'descubrams', 'ms', 'descubramatogrossodosul', etc.
      const isDescubraMS =
      ...
      // Descubra MS não exige mais "Complete seu perfil".
      if (isDescubraMS) {
        console.log("🏛️ PROFILE CHECKER: Rota Descubra MS detectada, pulando verificação de perfil");
        return;
      }
