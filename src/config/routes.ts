...
export const MS_BASE_PATH = '/descubrams';

// Rota legada (mantida para compatibilidade e redirecionamento)
export const MS_LEGACY_PATH = '/ms';

// Rota antiga (mantida para compatibilidade e redirecionamento)
export const MS_OLD_PATH = '/descubramatogrossodosul';
...
export const isMSRoute = (pathname: string): boolean => {
  return pathname.startsWith(MS_BASE_PATH) || pathname.startsWith(MS_LEGACY_PATH) || pathname.startsWith(MS_OLD_PATH);
};
...
export const MS_ROUTES = {
  ...
  /** Mesmo que `home`; não existe rota `/descubrams/welcome` no app. */
  welcome: MS_BASE_PATH,
  sobre: msRoute('sobre'),
