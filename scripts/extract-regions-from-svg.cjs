...
const colorMapping = {
  ...
  // AZUL MARINHO: BONITO-SERRA DA BODOQUENA (cores mais escuras)
  ...
  // AZUL FORTE/CIANO: VALE DAS ÁGUAS (cores mais claras, ciano)
...
// Extrair paths por cor - busca por fill e stroke
const regionPaths = {};

Object.keys(colorMapping).forEach(regionId => {
  const colors = colorMapping[regionId];
  const paths = [];
  const usedPaths = new Set(); // Para evitar duplicatas
  
  colors.forEach(color => {
    // Buscar por fill
    const fillRegex = new RegExp(`<g[^>]*fill="#${color}"[^>]*>([\\s\\S]*?)</g>`, 'gi');
    let match;
