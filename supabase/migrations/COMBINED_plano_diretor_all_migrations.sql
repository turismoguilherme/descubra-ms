...
CREATE TABLE IF NOT EXISTS plano_diretor_documents (
    ...
    criador_id UUID NOT NULL, -- Foreign key será adicionada depois (sem restrição para permitir usuários de teste)
...
-- NOTA: A foreign key para criador_id não é adicionada aqui para permitir usuários de teste
-- Em produção, você pode adicionar a constraint se necessário:
-- ALTER TABLE plano_diretor_documents ADD CONSTRAINT plano_diretor_documents_criador_id_fkey 
-- FOREIGN KEY (criador_id) REFERENCES auth.users(id) ON DELETE SET NULL;
...
-- ============================================================
-- PARTE 4: AJUSTES PARA USUÁRIOS DE TESTE (DESENVOLVIMENTO)
-- ============================================================

-- Remover foreign key restritiva para permitir usuários de teste
-- Isso permite inserir UUIDs que não existem em auth.users (para desenvolvimento)
ALTER TABLE plano_diretor_documents 
DROP CONSTRAINT IF EXISTS plano_diretor_documents_criador_id_fkey;

-- Remover outras foreign keys restritivas (opcional)
...
-- As foreign keys restritivas foram removidas para permitir usuários de teste.
-- 
-- NOTA: Em produção, você pode recriar as foreign keys se necessário:
-- ALTER TABLE plano_diretor_documents ADD CONSTRAINT plano_diretor_documents_criador_id_fkey 
-- FOREIGN KEY (criador_id) REFERENCES auth.users(id) ON DELETE SET NULL;
-- ============================================================
