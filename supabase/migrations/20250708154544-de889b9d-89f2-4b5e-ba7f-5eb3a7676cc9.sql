...
-- 3. Criar usuários de teste com diferentes roles (você pode usar estes emails para testar)
-- Nota: Os usuários serão criados via interface, mas as roles serão definidas aqui

-- Limpar roles existentes de teste (se houver)
DELETE FROM public.user_roles WHERE role IN ('test_admin', 'test_municipal', 'test_atendente');

-- 4. Padronizar as roles - vamos usar apenas estas roles oficiais:
-- 'admin', 'tech', 'municipal_manager', 'atendente', 'gestor', 'user'

-- 5. Criar function para automatizar redirecionamento baseado na role
...
-- 6. Criar function para validar se usuário tem permissão para acessar uma rota
