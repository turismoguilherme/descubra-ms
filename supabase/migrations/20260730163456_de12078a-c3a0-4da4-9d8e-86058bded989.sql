-- 1. Modo de ordem dos checkpoints por rota
ALTER TABLE public.routes
  ADD COLUMN IF NOT EXISTS checkpoint_order_mode text NOT NULL DEFAULT 'sequential';

ALTER TABLE public.routes
  DROP CONSTRAINT IF EXISTS routes_checkpoint_order_mode_check;
ALTER TABLE public.routes
  ADD CONSTRAINT routes_checkpoint_order_mode_check
  CHECK (checkpoint_order_mode IN ('sequential', 'free'));

-- 2. Opt-out do ranking
ALTER TABLE public.user_profiles
  ADD COLUMN IF NOT EXISTS leaderboard_opt_out boolean NOT NULL DEFAULT false;

-- 3. validate_and_stamp_checkpoint respeitando ordem livre
CREATE OR REPLACE FUNCTION public.validate_and_stamp_checkpoint(p_user_id uuid, p_checkpoint_id uuid, p_route_id uuid, p_latitude double precision, p_longitude double precision, p_photo_url text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
AS $function$
  WITH filtered AS (
    SELECT ps.user_id AS uid,
           COALESCE(ps.points_earned, 10) AS pts
  ...
  SELECT
    ...
    a.uid,
    COALESCE(NULLIF(up.full_name, ''), 'Viajante'),
    up.avatar_url,
    a.pts_total,
    a.stamps_total
  FROM agg a
...
AS $function$
  WITH filtered AS (
    SELECT ps.user_id AS uid, COALESCE(ps.points_earned, 10) AS pts
    FROM passport_stamps ps
    JOIN routes r ON r.id = ps.route_id
    WHERE ps.route_id IS NOT NULL
      AND (p_period <> 'month' OR ps.stamped_at >= date_trunc('month', now()))
...
REVOKE ALL ON FUNCTION public.get_passport_leaderboard(text, text, integer) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.get_my_leaderboard_position(text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_passport_leaderboard(text, text, integer) TO anon, authenticated;
