...
CREATE TABLE IF NOT EXISTS public.partner_transactions (
  ...
  transaction_type TEXT NOT NULL CHECK (transaction_type IN ('subscription_payment', 'commission', 'refund', 'payout')),
...
COMMENT ON COLUMN public.partner_transactions.transaction_type IS 'Tipo: subscription_payment (pagamento de assinatura), commission (comissão de reserva), refund (reembolso), payout (repasse para parceiro)';
COMMENT ON COLUMN public.partner_transactions.amount IS 'Valor da transação (positivo para receitas, negativo para despesas)';
COMMENT ON COLUMN public.partner_transactions.stripe_transfer_id IS 'ID da transferência Stripe Connect (se repasse automático)';
...
-- Política: Sistema pode atualizar transações (via service role)
