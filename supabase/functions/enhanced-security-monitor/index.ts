...
serve(async (req) => {
  ...
  try {
    ...
    const { data: isAdmin } = await supabaseClient.rpc('is_admin_user', { check_user_id: user.id });
    if (!isAdmin) {
    ...
    console.error('enhanced-security-monitor: handler error', { message: error?.message, stack: error?.stack });
    return new Response(
